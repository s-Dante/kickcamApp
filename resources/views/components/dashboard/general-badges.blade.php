...
@php
    $classes = [
        ...
        'badge' => [
            'unlocked' => [
                'container' => 'w-full h-full flex flex-col items-center p-3 rounded-xl bg-accent-desat/30 border border-accent/20 shadow-sm transition-transform cursor-pointer',
            ...
            'locked' => [
                'container' => 'w-full h-full flex flex-col items-center p-3 rounded-xl bg-tertiary-desat/10 border border-transparent',
...
<div class="{{ $classes['card'] }}">
    <div class="{{ $classes['card_body'] }}">
        <div class="{{ $classes['header']['container'] }}">
            <x-tooltip text="Aquí verás los logros desbloqueados por participar en la app" position="right">
        ...
        <div class="{{ $classes['grid'] }}">
            @foreach($generalBadges as $badge)
                ...
                @if($isUnlocked)
                    <!-- Logro Desbloqueado -->
                    <x-tooltip text="{{ $badge->description }}" position="top" class="block w-full h-full hover:scale-105 transition-transform">
                        <div class="{{ $classes['badge']['unlocked']['container'] }}">
                ...
                @else
                    <!-- Logro Bloqueado -->
                    <x-tooltip text="Sigue coleccionando para descubrirlo" position="top" class="block w-full h-full">
                        <div class="{{ $classes['badge']['locked']['container'] }}">
